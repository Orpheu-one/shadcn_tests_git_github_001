...
import BadgeBar from '@/components/gamification/BadgeBar'
...
const Navbar = () => {
  ...
  // 🎯 Determinar se mostra badges (só operadores e vendedores)
  const normalizedRole = userRole?.toLowerCase()
  const showBadges = normalizedRole && ['operator', 'd2d', 'vendedor'].includes(normalizedRole)
  ...
  return (
    <nav className='flex justify-between items-center px-4 py-2 bg-gray-200 dark:bg-transparent dark:text-slate-100'>
      ...
      <div className="flex items-center flex-row gap-4">
        ...
        {/* 🔥 BADGES - Lado Esquerdo (depois do logo) */}
        {showBadges && (
