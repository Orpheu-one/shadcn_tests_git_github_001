    ...
    <tr key={item.id} className="border-b border-gray-500 even:bg-purple-300 hover:bg-purple-500">
        ...
        <td className=""> 
            <div className="flex items-center gap-2">
                {/* Botão de Edição - Passamos userRole="ADMIN" */}
                <FormModal table="administradores" type="edit" userRole={UserRole.ADMIN} /> 
            
                {/* Botão de Eliminar (apenas para Admin) - Passamos userRole="ADMIN" */}
                {currentUserRole === "admin" && (
                    <FormModal table="administradores" type="delete" id={item.id} userRole={UserRole.ADMIN} />
                )}
            </div>
        </td>
    </tr>
...
const AdministradoresPage = async ({ searchParams }: SearchProps) => {
  ...
  const combinedWhere = {
    role: UserRole.ADMIN, // <-- FILTRO APLICADO PARA ADMINISTRADOR!
    ...searchFilter,           
  };

    // --- 3. CONSULTA DOS DADOS ---
    const administradores: UserWithEvents[] = await prisma.user.findMany({
        where: combinedWhere,
        include: {
            events: true, 
        },
        ...
        return (
            <div className="p-8 text-center bg-gray-100 text-gray-700 rounded-lg m-4 mt-0">
                <h2 className="text-xl font-bold">
                    {query.length > 0 ? `Nenhum Administrador encontrado para "${query}"` : "Nenhum Administrador Encontrado"}
                </h2>
                <p>Cria o primeiro Administrador para começar!</p>
                {currentUserRole === "admin" && (
                    <div className="mt-4 inline-block">
                        {/* Botão de Criação - Passamos userRole="SUPERVISOR" */}
                        <FormModal table="administradores" type="create" userRole={UserRole.SUPERVISOR} />
                    </div>
                )}
            </div>
        );
    ...
    return (
        <div className=' flex-1 bg-white p-4 rounded-lg m-4 mt-0'>
            ...
            <div className="flex items-center justify-between ">
                ...
                <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
                    ...
                    <div className="flex items-center gap-4 self-end">
                        ...
                        {currentUserRole === "admin" && (
                            // Botão de Criação - Passamos userRole="ADMIN"
                            <FormModal table="administradores" type="create" userRole={UserRole.ADMIN} />
                        )}
                    </div>
                </div>
            </div>
            ...
            <div className="text-black overflow-x-auto">
                <Table columns={columns} renderRow={renderRow} data={administradores} />
            </div>
