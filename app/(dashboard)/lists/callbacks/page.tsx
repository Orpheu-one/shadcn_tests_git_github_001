...
const columns = [
  { header: "ID Callback", accessor: "event_id" }, // ✅ Alterado de "ID Venda"
  ...
  { header: "Agendado", accessor: "scheduled", className: "hidden lg:table-cell" }, // ✅ Novo campo
...
const renderRow = (item: EventWithRelations) => (
  ...
  >
    ...
    {/* ✅ NOVO: Data agendada para callback */}
    ...
    {/* COLUNA STATUS com código de cores */}
...
const CallbacksPage = async ({ searchParams }: SearchProps) => {
  ...
  // ✅ CORRIGIDO: Query filtra apenas CALLBACKS
  const callbacks = await prisma.event.findMany({
    ...
    where: {
      type: 'CALLBACK' // ✅ Campo correto é "type", não "EventType"
    },
    take: ITEMS_PER_PAGE,
  ...
  // ✅ CORRIGIDO: Count apenas de callbacks
    ...
    <div className='flex-1 bg-white p-4 rounded-lg m-4 mt-0'>
      <div className="flex items-center justify-between mb-4">
        <h1 className="hidden md:block text-lg font-semibold text-black">
          Lista de Callbacks {/* ✅ Título corrigido */}
        </h1>
